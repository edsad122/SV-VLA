...
class L1RegressionActionHead(nn.Module):
    ...
    def predict_action(self, actions_hidden_states):
        ...
        if actions_hidden_states.shape[1] % ACTION_DIM != 0:
            raise ValueError(
                f"Expected action-token length divisible by ACTION_DIM ({ACTION_DIM}), got {actions_hidden_states.shape[1]}"
            )
        chunk_len = actions_hidden_states.shape[1] // ACTION_DIM
        rearranged_actions_hidden_states = actions_hidden_states.reshape(batch_size, chunk_len, -1)
...
class DiffusionActionHead(nn.Module):
    ...
    def sample_noisy_actions(self, ground_truth_actions):
        ...
        chunk_len = ground_truth_actions.shape[1]
        # Sample random noise with shape equal to actions, used for closed-form forward diffusion.
        noise = torch.randn(size=(batch_size, chunk_len, ACTION_DIM), device=device, dtype=ground_truth_actions.dtype)  # (B, chunk_len, action_dim)
    ...
    def predict_noise(self, actions_hidden_states):
        ...
        chunk_len = actions_hidden_states.shape[1] // ACTION_DIM
        rearranged_actions_hidden_states = actions_hidden_states.reshape(batch_size, chunk_len, -1)  # (batch_size, chunk_len, action_dim * hidden_dim)
