...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward_to_penultimate(
        ...
        output_attentions: Optional[bool] = False,
        cache_position: Optional[torch.LongTensor] = None,
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.LongTensor, Optional[Cache], Optional[torch.LongTensor]]:
        """Runs all decoder layers except the final layer and returns hidden states and masks needed for the last layer."""
        ...
        if len(self.layers) <= 1:
            return hidden_states, causal_mask, position_ids, past_key_values, cache_position
    ...
    def forward_last_layer(
        ...
        """Runs only the final decoder layer and final RMSNorm."""
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def __init__(self, config, attn_implementation = None):
        # 2. 核心魔法：如果外部传了注意力实现方式，就强制把它绑定到 config 的隐藏属性上
        if attn_implementation is not None:
            config._attn_implementation = attn_implementation
        # 兜底机制：如果没传，且 config 里本来也没有，就默认用基础版本
        elif not hasattr(config, "_attn_implementation"):
