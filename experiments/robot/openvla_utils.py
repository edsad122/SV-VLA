...
def update_auto_map(pretrained_checkpoint: str) -> None:
    ...
    # Read and update the config; recover from backup if current config.json is malformed.
    config = None
    try:
        with open(config_path, "r", encoding="utf-8") as f:
            config = json.load(f)
    except json.JSONDecodeError as err:
        print(f"Warning: malformed config.json at {config_path}: {err}")
        backup_candidates = sorted(
            [
                os.path.join(pretrained_checkpoint, p)
                for p in os.listdir(pretrained_checkpoint)
                if p.startswith("config.json.back.")
            ],
            reverse=True,
        )
        ...
        for bp in backup_candidates:
            try:
                with open(bp, "r", encoding="utf-8") as f:
                    config = json.load(f)
                print(f"Recovered config from backup: {os.path.abspath(bp)}")
                break
        ...
        if config is None:
            raise RuntimeError(
                f"Failed to parse {config_path} and no valid backup config.json.back.* was found."
            ) from err
...
def get_vla_action(
    ...
    with torch.inference_mode():
        ...
        # Generate action
        reset_first_frame_cache = bool(obs.get("reset_first_frame_cache", False) or obs.get("is_first_frame", False))
        if action_head is None:
            ...
            action, _ = vla.predict_action(
                **inputs,
                ...
                do_sample=False,
                reset_first_frame_cache=reset_first_frame_cache,
            )
        else:
