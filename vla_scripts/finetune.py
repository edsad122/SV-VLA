...
class FinetuneConfig:
    ...
    use_temporal_fusion: bool = False                # If True, enable (first-frame feat || frame-t ViT-T feat) temporal fusion
    vit_t_timm_model_id: str = "vit_tiny_patch16_224.augreg_in21k_ft_in1k"  # TIMM model id for frozen ViT-T branch
    vit_t_image_size: int = 224                      # Input resolution for ViT-T branch
    train_last_layer_and_fusion_only: bool = False   # If True, train only final LLaMA layer + temporal fusion FC
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    # LoRA setup
    if cfg.use_lora and not cfg.train_last_layer_and_fusion_only:
    ...
    if cfg.train_last_layer_and_fusion_only:
        for param in vla.parameters():
            param.requires_grad = False

        for param in vla.language_model.model.layers[-1].parameters():
            param.requires_grad = True
        logger.info(
            "`train_last_layer_and_fusion_only=True`: training only LLaMA last layer. "
            "Temporal fusion FC is externalized and no longer lives inside `OpenVLAForActionPrediction`."
        )
